...
function dayTagFormat(year, month, day) {
	...
	tag.append("</td>"); // 실제 화면에 ex)23 앵커태그
	return tag.toString();
}
//숫자 5 -> 05 변경
function numFormat(num) {
	var str = ''+num;
	if(num<10 && str.indexOf('0') == -1) { //[메소드 결과=-1] 0이 없는지 찾아보기
		str = '0'+num;
	}
	return str;
}
...
function monthDayIndex(month, day) { //여기선 month가 배열임
	for(var i=0; i<month.length; i++) {   //이 배열의 길이는 최대 31
		if(month[i]==day) {  
			return i; //결국엔 int형 반환인디.. 알것다
		}
	}
}
...
function screenWriteMonth() {
	...
	var months = [month-1, month, month+1]; //공부
	if(month == 1) {
	...
	var monthDay = calendar.make(year, months[1]); //현재달력의 날짜 위치를 계산한 배열임
	var tag = new StringBuffer(); 
	var startIndex = monthDayIndex(monthDay, 1); //2018/11/1의 현재위치의 요일을 구함 최소 0 최대 5
	var lastIndex = monthDayIndex(calendar.makeOne(year, months[1]), calendar.lastDay(year, months[1])) + startIndex;
	//매개변수(2018년 11월  ,11월이면 30)의  위치 반환 + 2018/11/1의 현재위치의 요일을 구함 최소 0 최대 5 => 위치지정 4부터 
	for(var i=0; i<monthDay.length; i++) {
		if(i%7 == 0) {
			tag.append('<tr>');
		}
		if(i<startIndex) { //화면에 먼저 출력 2018.11기준 윗1행의 1열부터 4개출력
			if(months[0]==12) { //month[0,1,2] 0==12 1월달을 의미 
				tag.append(dayTagFormat(year-1, months[0], monthDay[i])); //지난달
			}else {
				tag.append(dayTagFormat(year, months[0], monthDay[i]));
			}
		}else if(i <= lastIndex) { //그 다음 화면 2018.11기준  윗1행 4열부터 출력 lastIndex값은 30임 
			tag.append(dayTagFormat(year, months[1], monthDay[i])); //현재달
		}else {
			if(months[2]==1) { //12월달을 의미 
				tag.append(dayTagFormat(year+1, months[2], monthDay[i])); //다음달
			}else {
...
// 일정 태그를 만들어 준다 일정태그를 만들어봐야 알 수 있을 듯
